                        ...
                        <transition name="topSlide" @before-enter="beforeEnter" @enter="enter" @before-leave="beforeLeave" @leave="leave">
                            <div v-show="isOpen[0]" class="topSlide">
                                ...
                                <p>
                                    ...
                                    　そこで、まず学習のモデルを変更しYOLOから軽量化されたYOLO、それも上手くいかず...を繰り返し、最終的に「<a href="https://github.com/EdjeElectronics/TensorFlow-Lite-Object-Detection-on-Android-and-Raspberry-Pi/tree/master">TensorFlow-Lite</a>」というモデルを扱うことになりました。
                                    ...
                                    　ロボコンが進んでいくにつれて、やっぱり10fps以上で認識率9割以上は欲しいよねという話と安定性の面(ラズパイだと長時間の認識処理は排熱などでパフォーマンスが低下する可能性)の話があり、最終的には機体に搭載するモジュールをラズパイからJetsonというものに変更しました。
                        ...
                        <transition name="topSlide" @before-enter="beforeEnter" @enter="enter" @before-leave="beforeLeave" @leave="leave">
                            <div v-show="isOpen[1]" class="topSlide">
                                <p>
                                    ...
                                    実際は、windows上でLinuxを扱えるWSLで作業をしていましたが、ここでの環境構築には結構苦労しました。
